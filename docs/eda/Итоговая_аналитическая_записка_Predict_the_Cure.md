...
**Связанные артефакты:** автоматический EDA [`EDA_Report.md`](EDA_Report.md), архитектурная экспертиза [`Аналитическая_записка_архитектура_моделей.md`](Аналитическая_записка_архитектура_моделей.md), ревью EDA домена [`Ревью_eda_1.md`](Ревью_eda_1.md), ML-инженерный обзор улучшений [`ML_инженерный_обзор_улучшения_прогноза.md`](ML_инженерный_обзор_улучшения_прогноза.md).
...
Ранее в репозитории поддерживался вспомогательный скрипт в каталоге `tools/` (**сериализация отправок №2–5 с OOF-stacking**, SI-blend, GroupKFold, опционально Optuna). По состоянию текущего рабочего дерева **`tools/` в корне удалён** (узкий склад: только базовый `train`/`predict`); описание серии сохранено как историческая база ниже (`submission*_a4.csv`), фактические CSV при необходимости восстановить из сохранённых копий/веток git.
...
- фактический CSV — из локально сохранённого файла `submission2_a4.csv` или из истории коммитов/ветки, где скрипт оркестрации ещё присутствовал (повтор воспроизведения возможен после восстановления кода генератора или ручным повторением схемы по §5).
...
- авторская цепочка stacking/SI описана в документации и при необходимости может быть воссоздана отдельно; поддерживаемый путь сборки только **`train.py`/`predict.py`**.  
...
Перечень согласован с [`Аналитическая_записка_архитектура_моделей.md`](Аналитическая_записка_архитектура_моделей.md) и [`ML_инженерный_обзор_улучшения_прогноза.md`](ML_инженерный_обзор_улучшения_прогноза.md):
