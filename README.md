...
Убедитесь, что в **`data`** лежат `train.csv` и `test.csv` (корень проекта — снова относительно путём `.\data\`):
...
Должно вывестись `True` для обоих. Если файлов нет — скопируйте выдачу соревнования в `.\data\`.
