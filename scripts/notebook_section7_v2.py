# Ячейки §7 для notebooks/39_chemai_v2.ipynb — author4 submission2 (Kaggle ~326.89)
...
**Схема:** ClusterKFold → OOF 9 моделей → Ridge meta-stacking = **`submission2_a4`** (Kaggle **326.89**).

**§7.1–§7.9** — OOF каждой модели (аналитика). **§7.10** — **канонический** stacking как `author4 --only 2` (именно он даёт эталонный CSV).
...
def pred_to_original(pred, use_log):
    """Обратно в исходную шкалу — как author4 / chemai (без лишнего clipping)."""
    if use_log:
...
def collect_oof_multi(x_raw, y_raw, candidates, cfg, use_log):
    """
    OOF всех моделей за один проход CV — как collect_oof в author4 submission2.
    Именно эта матрица используется для финального submission (§7.10).
    """
    n = len(x_raw)
    c = len(candidates)
    ...
    for fold_id, (tr, va) in enumerate(cv.split(x_raw, y_raw)):
        ...
        for j, cand in enumerate(candidates):
            m = cand.fit_fold(x_tr, y_s[tr], x_va, y_s[va], rs)
            oof[va, j] = pred_to_original(m.predict(x_va), use_log)

    if np.isnan(oof).any():
        raise RuntimeError("NaN в OOF-matrix — проверьте данные и Restart runtime")
    return oof


def fit_finals_for_target(x_full_np, y_raw, candidates, cfg, use_log):
    """Финальные модели на полном train — как fit_final_models_for_target в author4."""
    y_s = y_train_space(y_raw, use_log)
    out = {}
    for cand in candidates:
        inner = fit_all_final(cand, x_full_np, y_s, cfg.random_seed)
...
SECTION7_STACKING = r'''
# @title §7.10. Ridge meta-stacking → submission (author4 submission2)
"""
Канонический stacking = tools/author4_high_priority_submissions.py --only 2.
§7.1–§7.9 — аналитика OOF; submission_df строится здесь заново (author4-путь).
"""
import time

_prereq = (
    "OOF_STORE", "CANDIDATE_ORDER", "MODEL_METRICS", "CANDIDATES",
    "y_df", "x_train_feat", "x_full_np", "x_test_np", "idx", "CFG",
    "collect_oof_multi", "fit_meta_ridge", "fit_finals_for_target",
    "postprocess", "competition_score", "Expm1Predictor", "TARGETS", "INDEX_COL",
)
_missing = [n for n in _prereq if n not in globals()]
if _missing:
    raise RuntimeError(
        "Не запущены §7.0–§7.9. Нет: "
        + ", ".join(_missing[:8])
        + ("…" if len(_missing) > 8 else "")
        + ". Colab: Restart → Run All с §1."
    )
...
print("OOF базовых моделей (§7.1–§7.9, competition_score):")
display(base_oof_df)

cand_list = [CANDIDATES[n] for n in CANDIDATE_ORDER]
metas, oof_pred, finals = {}, {}, {}

for t in TARGETS:
    print(f"[{time.strftime('%H:%M:%S')}] author4 OOF + finals: {t} …", flush=True)
    y_raw = y_df[t].to_numpy(dtype=np.float64)
    use_log = CFG.log_transform_ic50_cc50 and t in ("IC50", "CC50")
    oof = collect_oof_multi(x_train_feat, y_raw, cand_list, CFG, use_log)
    meta = fit_meta_ridge(oof, y_raw)
    metas[t] = meta
    oof_pred[t] = meta.predict(oof)
    finals[t] = fit_finals_for_target(x_full_np, y_raw, cand_list, CFG, use_log)
...
print(f"[{time.strftime('%H:%M:%S')}] predict test …", flush=True)
test_stack = {}
for t in TARGETS:
    cols = [finals[t][c.name].predict(x_test_np) for c in cand_list]
...
submission_df = postprocess(
    pd.DataFrame({INDEX_COL: idx, "IC50": ic_t, "CC50": cc_t, "SI": si_t})
)

EXPECTED_STACK_OOF = 557.2170244090267
if abs(stack_oof - EXPECTED_STACK_OOF) >= 1.0:
    raise AssertionError(
        f"OOF stacking {stack_oof:.4f} ≠ author4 {EXPECTED_STACK_OOF:.4f}. "
        "Restart runtime → Run All с §1."
    )

print("OOF stacking competition_score:", round(stack_oof, 4))
print("OOF RMSE по таргетам:", {k: round(v, 2) for k, v in stack_parts.items()})
print("Submission preview (author4 submission2):")
submission_df.head()
'''
