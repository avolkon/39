#!/usr/bin/env python
"""Generate standalone notebooks/39_chemai_v2.ipynb (author4 submission2, Kaggle 326.89)."""
...
OUT = ROOT / "notebooks" / "39_chemai_v2.ipynb"

_conc: dict = {}
exec((ROOT / "scripts" / "notebook_conclusions_v2.py").read_text(encoding="utf-8"), _conc)

_s7: dict = {}
exec((ROOT / "scripts" / "notebook_section7_v2.py").read_text(encoding="utf-8"), _s7)
...
TITLE = """\
# Хакатон ChemAI: Predict the Cure (v2 — author4 submission2)
...
**Kaggle:** ChemAI: Predict the Cure, команда 39

https://www.kaggle.com/competitions/chem-ai-predict-the-cure/overview

**Цель v2:** Run All → CSV = **`submission2_a4`** (Kaggle **326.89**).  
Пайплайн выровнен с `tools/author4_high_priority_submissions.py --only 2`.

⚠️ **Colab:** Runtime → **Restart session** → **Run All** с §1. §9 сохраняет **`39_chemai_submission.csv`** для Kaggle.
"""

CELLS = [
    md(TITLE),
    code(
        """# @title Установка зависимостей (фиксированные версии)
\"\"\"
Воспроизводимость author4 submission2: согласованный numpy+scipy+sklearn.
Colab: после первой установки runtime перезапустится автоматически — затем Run All.
\"\"\"
import subprocess
import sys
...
try:
    import sklearn
    import numpy as np
    if sklearn.__version__ == TARGET_SKLEARN and np.__version__.startswith("1.26."):
        need_install = False
except Exception:
    pass

if need_install:
    print("Установка согласованного стека (numpy 1.26 + scipy 1.11 + sklearn 1.4.2)…")
    subprocess.check_call(
        [sys.executable, "-m", "pip", "install", "-q", "--force-reinstall", *STACK]
    )
    print("Готово. Перезапуск runtime (Colab) — затем Run All с §1.")
    try:
        import IPython
        ...
        pass
    raise SystemExit(
        "Пакеты установлены. Runtime → Restart session → Run All (§0 пройдёт без reinstall)."
    )

import numpy as np
import sklearn
print("Зависимости OK | numpy", np.__version__, "| sklearn", sklearn.__version__)
"""
    ),
    code(
...
"""
    ...
    md(
        ...
        "- Положите CSV рядом с ноутбуком или в `data/`, либо загрузите через Colab (§2).\n"
        "- **§9** сохраняет `39_chemai_submission.csv` после §7.10.\n"
        "- Не запускайте §9 отдельно без `submission_df` из §7.10."
    ),
    code(
...
"""
    ...
    code(
        """# @title §3. EDA — сводная статистика и асимметрия таргетов
\"\"\"
Сводная статистика таргетов IC50, CC50, SI.
Асимметрия > 0 → правый хвост (обоснование log1p в §7).
\"\"\"
_COL_RU = {
    "count": "количество",
    "mean": "среднее",
...
desc = desc.rename(columns=_COL_RU)
...
\"\"\"
Сохраняет submission_df из §7.10 в CSV для Kaggle.
\"\"\"
from pathlib import Path

if "submission_df" not in globals():
    raise RuntimeError("Сначала Run All до §7.10 — нужен submission_df.")

out = Path("39_chemai_submission.csv")
assert submission_df.shape == (250, 4)
assert list(submission_df.columns) == ["index", "IC50", "CC50", "SI"]
assert submission_df[["IC50", "CC50", "SI"]].min().min() >= 0

submission_df.to_csv(out, index=False)
print("Сохранён:", out.resolve())
if "stack_oof" in globals():
    print(f"stack_oof = {stack_oof:.4f}")
